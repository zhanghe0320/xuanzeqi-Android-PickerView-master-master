...
import com.bigkoo.pickerviewdemo.bean.JsonBean2;
...
public class JsonDataActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ArrayList<JsonBean2> options1Items2 = new ArrayList<>();
    ...
    private void showPickerView() {// 弹出选择器
        ...
        pvOptions.setPicker(options1Items2, options2Items, options3Items);//三级选择器
        pvOptions.show();
    }
    ...
    private void initJsonData() {//解析数据
        ...
        String JsonData2 = new GetJsonDataUtil().getJson(this, "address.json");//获取assets目录下的json文件数据

        ArrayList<JsonBean2> jsonBean2 = parseData2(JsonData2);//用Gson 转成实体
        /**
         * 添加省份数据
         *
        ...
        options1Items2 = jsonBean2;


           for (int i = 0; i < jsonBean2.size(); i++) {//遍历省份
            ArrayList<String> CityList = new ArrayList<>();//该省的城市列表（第二级）
            ArrayList<ArrayList<String>> Province_AreaList = new ArrayList<>();//该省的所有地区列表（第三极）

            for (int c = 0; c < jsonBean2.get(i).getChilds().size(); c++) {//遍历该省份的所有城市
                String CityName = jsonBean2.get(i).getChilds().get(c).getName();
                CityList.add(CityName);//添加城市
                ArrayList<String> City_AreaList = new ArrayList<>();//该城市的所有地区列表

                //如果无地区数据，建议添加空字符串，防止数据为null 导致三个选项长度不匹配造成崩溃
                if (jsonBean2.get(i).getChilds().get(c).getChilds() == null || jsonBean2.get(i).getChilds().get(c).getChilds().size() == 0) {
                    City_AreaList.add("");
                } else {

                    for(int d = 0; d <jsonBean2.get(i).getChilds().get(c).getChilds().size();d++){
                        City_AreaList.add(jsonBean2.get(i).getChilds().get(c).getChilds().get(d).getName());
                    }
...
//
    ...
    public ArrayList<JsonBean2> parseData2(String result) {//Gson 解析
